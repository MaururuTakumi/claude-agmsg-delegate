...
最終Skill dry-runの前に、~/.agents/skills/agmsg/scripts/ のwhoami.sh、send.sh、list-ids.shが存在し、実行可能か確認してください。
agmsgが未導入なら停止し、npx agmsgの実行、team参加、delivery mode選択の前に私へ明示承認を求めてください。設定を推測したりagmsgのファイルを直接編集したり、api.shの互換fallbackを追加したりしないでください。
...
wrapperだけがagmsgの`whoami.sh`、`send.sh`、`list-ids.sh`を呼びます。Claudeにはagmsg toolを渡さず、DBやteamファイルを直接読みません。
...
`api.sh`などを互換fallbackとして追加・使用してはいけません。
