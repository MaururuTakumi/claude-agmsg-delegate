...
delegate_claude.py --versionが0.3.1であることを確認してください。
...
version `0.3.1`のruntime contractは`contract_version=2`です。Fable／Sonnetの
...
上記の`./install.sh --force`を再実行し、表示されたversionが`0.3.1`である
...
期待値は`0.3.1`で、dry-runには`contract_version: 2`が出ます。実jobがfileを
