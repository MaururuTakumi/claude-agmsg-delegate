...
delegate_claude.py --versionが0.4.0であることを確認してください。
...
version `0.4.0`のruntime contractは`contract_version=3`です。Fable／Sonnetの
...
上記の`./install.sh --force`を再実行し、表示されたversionが`0.4.0`である
...
期待値は`0.4.0`で、dry-runには`contract_version: 3`が出ます。実jobがfileを
