...
Claude側の処理がすべてread-onlyなのではありません。

| 呼び出し | mode | Claudeへ渡すtools | file編集 |
| --- | --- | --- | --- |
| roleを問わないFable | `workspace_read` | `Read,Glob,Grep` + `plan` | 不可。hard read-only |
| 相談だけのSonnet | `workspace_read` | `Read,Glob,Grep` + `plan` | 不可 |
| 明示的なSonnet implementer | `workspace_write` | `Read,Edit,Write,Glob,Grep` + `acceptEdits` | `--workspace-write`指定時だけ可 |

role名だけでは書き込み権限を取得できません。Fableへ
`--role implementer`を指定してもread-onlyのままで、Fableと
`--workspace-write`を組み合わせた依頼はagmsg送信やClaude推論より前に拒否
されます。書き込み可能なのは
`--model sonnet --role implementer --workspace-write`をすべて明示した場合だけ
です。どのmodeでもBashは利用できず、commandとテスト、diffレビューはCodexが
担当します。

Fableは`--github-issue`で明示指定したGitHub Issueもレビューできます。
wrapperが端末の認証済み`gh` CLIから対象Issueだけをread-onlyで取得し、
タイトル、本文、ラベル、コメントのbounded snapshotをFableへ渡します。
Fable自身にBash、GitHub認証情報、通常のClaude settings、GitHub書き込みtoolは
渡しません。

### 未公開repositoryと承認

agmsgが運ぶのはtaskと相関済みresultであり、repository自体をuploadするもの
ではありません。実jobでは、wrapperが端末へインストール済みのClaude Codeを
新しい制限付きprocessとして起動し、検証済みClaude.ai subscriptionを使います。
そのため、関連する`Read`、`Glob`、`Grep`の結果は、同じprojectでterminalの
Claude Codeを使う場合と同様にClaude.aiで処理されます。一方、terminalの会話、
user settings、Bash、無制限の権限は継承しません。

ユーザーがFableへcurrent projectのreviewを明示依頼した時点で、通常の関連
fileを1回bounded readする承認として十分です。private workspace packetと除外
対象がすでに承認されている場合は、その除外を維持して実行し、一般的な
「外部Claudeへの送信承認」を重ねて聞きません。「FableもCodexと同じ権限で
よい」はFableの最大許可である`workspace_read`へmapし、writeやshell権限は
追加しません。

`delegate_claude.py`を実行して具体的なerrorが返っていない限り、安全機構が
拒否したと報告してはいけません。wrapperを呼ばなかった場合はorchestratorの
判断であり、外部拒否の証拠ではありません。
dry-runは機械可読な`data_flow`も返し、agmsgがtask/result envelopeだけを運ぶ
こと、Claude processがlocal headlessであること、real jobのtool結果は
Claude.aiで処理されること、dry-run自身はworkspace contentを送っていないこと
を区別します。
...
delegate_claude.py --versionが0.4.1であることを確認してください。
...
- 任意: Fableの`--github-issue`入力を使う場合は、認証済みGitHub CLI
...
Codex Desktop
  ...
  ├─ 任意のIssue context: 認証済みgh issue view
  │    └─ 明示指定した参照だけをread-only取得。tokenはClaudeへ渡さない
...
version `0.4.1`のruntime contractは`contract_version=3`です。Fable／Sonnetの
...
python3 "${CODEX_HOME:-$HOME/.codex}/skills/claude-agmsg-delegate/scripts/delegate_claude.py" run \
  ...
  --task "Issueの要件と設計上のriskをレビューしてください。" \
  --timeout 60
```

`--github-issue`には`NUMBER`、`OWNER/REPO#NUMBER`、またはgithub.comのIssue
URLを指定でき、最大5件まで繰り返せます。明示確認が必要なのは、一般の文章に
患者情報や不要な個人情報があるかを自動判定できないためです。comment authorの
identityはcontextから除外します。wrapperはargument arrayで`gh issue view`を
実行し、そのsubprocessへ`GH_TOKEN`と`GITHUB_TOKEN`を渡しません。`gh`は端末の
既存loginを使います。
...
`--dry-run`はlocalのread-only認証確認だけを実行します。model inference、
agmsg送信、job state作成、指定したGitHub Issueのnetwork取得は行いません。
...
`--github-issue`を使ったFable jobでは、completed resultに
`github_issues_read`（例：`["owner/repo#123"]`）と
`github_context_source: "authenticated_gh_cli"`も含まれます。
...
- permission bypassなし
- secretらしい依頼は送信前に拒否
- GitHub Issue contextは明示指定・安全確認済みの対象だけをread-only取得
- FableへBash、GitHub token、GitHub書き込みtoolを渡さない
...
上記の`./install.sh --force`を再実行し、表示されたversionが`0.4.1`である
...
期待値は`0.4.1`で、dry-runには`contract_version: 3`が出ます。実jobがfileを
